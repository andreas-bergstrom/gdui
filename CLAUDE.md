...
- **`internal/ui`** — the Bubble Tea program.
  ...
  - `r` triggers `loadInitDataCmd` which re-lists worktrees and reloads all sections in one synchronous batch — sections that survive (matched by `WT.Root`) preserve their per-node expand state via `preserveTreeState`. New worktrees added during a session won't gain a watcher until gdui is restarted (watchers are spawned once in `main.startWatchers`).
